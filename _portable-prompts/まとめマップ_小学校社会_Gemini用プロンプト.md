# 小学校社会「まとめマップ」生成プロンプト ＜Gemini用＞

## 使い方
1. 下の「=== プロンプト ここから ===」から「=== ここまで ===」までを **全部コピー** してGeminiに貼る
2. 続けて **教科書のページを写真またはPDFでアップロード** し、**「学年・単元名・ページ」** を伝える
3. 返ってきたコードブロックを `xxx.html` で保存 → ダブルクリックで開く（印刷もできる）
   - Geminiの **Canvas** をオンにすると、その場でプレビューできます。

※ Gem にする場合：本文を **指示（Instructions）** に、末尾の【手本テンプレート】HTMLを **ナレッジ（ファイル添付）** に入れると、毎回貼らずに済みます。
...
# ⚠️ 最優先（ここで失敗が起きやすい）
- **コードを絶対に省略しない**。`...`／「（以下同様）」／「省略」を使わず、すべてのノード・全行を完全なHTMLで書ききる。
- 出力の前に、頭の中で次の順に組み立てる：
  1. **語句リスト**（教科書から抜き出す重要語 10〜15個。地名・人物・年号などの固有名詞を含む）
  2. **配置表**を作る（各ノードについて「語句／x／y／幅／高さ」を一覧化）。**ノード同士・線が重ならないか座標で検算**してから描く。特に **大きなまとまり（大枝）どうしの間に50px以上のすき間**を空ける。
  3. 検算が済んでから **SVGを書く**。
- **並列（同列のなかま）は必ず横並び**にする。縦に積むと順序・段階に見えてしまう。
...
# 出力前に自分でチェック
- ノード同士・線が重ならない。テキストが rect 幅・viewBox 内に収まる。大枝どうしのすき間は50px以上。
...
# 最後にもう一度（重要）
- 返答は **HTMLのコードブロック1つだけ**。前後に説明文を書かない。
- **コードは省略しない**。途中で `...` や「省略」を使わず、最後の `</html>` まで完全に出力する。
