...
- 出力の前に、頭の中で次の順に組み立てる：
  1. **語句リスト**（教科書から抜き出す重要語 10〜15個）
  2. **配置表**を作る（各ノードについて「語句／x／y／幅／高さ」を一覧化）。**ノード同士・線が重ならないか座標で検算**してから描く。
  3. 検算が済んでから **SVGを書く**。
- 親が同じ「なかま」を並べるときは **櫛形**（下の【並列サンプル】の形）。直列の「→」で数珠つなぎにしない。
